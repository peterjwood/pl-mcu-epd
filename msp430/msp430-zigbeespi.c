...
#define	SPI_SIMO                MSP430_GPIO(4,4)
#define	SPI_SOMI                MSP430_GPIO(4,0)
#define	SPI_CLK                 MSP430_GPIO(4,5)

#endif

int msp430_zigbee_spi_read_bytes(uint8_t *buff, uint8_t size);
int msp430_zigbee_spi_write_bytes(uint8_t *buff, uint8_t size);
/* We only support a single SPI bus and that bus is defined at compile
 * time.
 */
int msp430_zigbee_spi_init(struct pl_gpio *gpio, uint8_t spi_channel, uint16_t divisor)
{
...
void msp430_zigbee_spi_close(void)
{
	UCxnCTL1 |= UCSWRST;                      // Put state machine in reset
}
