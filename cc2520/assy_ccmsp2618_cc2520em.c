...
* GLOBAL VARIABLES
*/
const digioConfig pinRadio_GPIO0 = {2, 0, BIT0, HAL_DIGIO_INPUT,  0};
...
static void halRadioSpiInit(uint32 divider)
{
    ...
    UCA2BR0 = LO_UINT32(divider);
    UCA2BR1 = HI_UINT32(divider);
    P9DIR |= BV(3);
    CC2520_ENABLE_SPI_FUNC();                    // P9.3,2,1 peripheral select (mux to ACSI_A0)
    UCA2CTL1 = UCSSEL0 | UCSSEL1;                // Select SMCLK
    UCA2CTL0 |= UCSYNC | UCCKPH | UCMSB | UCMST; // 3-pin, 8-bit SPI master, rising edge capture
